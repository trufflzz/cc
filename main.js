...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		Game.Prompt('<id ImportSave><h3>'+loc("Import save")+'</h3><div class="block">'+loc("Please paste in the code that was given to you on save export.")+'<div id="importError" class="warning" style="font-weight:bold;font-size:11px;"></div></div><div class="block"><textarea id="textareaPrompt" style="width:100%;height:128px;">'+(def||'')+'</textarea></div>',[[loc("Load"),'if (l(\'textareaPrompt\').value.length==0){return false;}if (Game.ImportSaveCode(l(\'textareaPrompt\').value)){Game.ClosePrompt();}else{l(\'importError\').innerHTML=\'(\'+loc("Error importing save")+\')\';}'],loc("Nevermind")]);//prompt('Please paste in the text that was given to you on save export.','');
			...
			Game.ConsolePrompt=function(def)
			{
				//if (App) return false;
			Game.Prompt('<id ImportSave><h3>'+loc("Console")+'</h3><div class="block">'+loc("Paste in your JavaScript code")+'<div id="importError" class="warning" style="color:white;font-size:11px;"></div></div><div class="block"><textarea id="textareaPrompt" style="width:100%;height:128px;">'+(def||'')+'</textarea></div>',[[loc("Execute"),'l(\'importError\').innerHTML="<br>"+loc(eval(l(\'textareaPrompt\').value));'],loc("Close")]);//prompt('Please paste in the text that was given to you on save export.','');
				l('textareaPrompt').focus();
			}
		...
		Game.UpdateMenu=function()
		{
			...
			if (Game.onMenu=='prefs')
			{
				...
				str+=
					...
					'<br><div class="listing"><a class="option smallFancyButton" '+Game.clickStr+'="Game.ConsolePrompt();PlaySound(\'snd/tick.mp3\');">'+loc("Console")+'</a><label>'+loc("Open a menu to execute JavaScript")+'</label></div>'+
